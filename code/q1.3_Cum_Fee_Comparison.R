...
Cum_Fee_Comparison_1.3 <- function(SWIX_funds, Fee = 0*1e-4, Start = ymd(20030101),
...
SWIX_funds <- BM %>%
    ...
    graph <-
        ...
        labs(title = "Fee Impact on Cumulated Wealth: SWIX",
             subtitle = glue::glue("Base Return: SWIX Performance | Start Date: {format(Start, '%B %Y')}"),
