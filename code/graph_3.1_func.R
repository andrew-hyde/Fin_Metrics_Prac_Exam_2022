...
graph_q3.1_func <- function(df_data, title, subtitle, caption, xlabel, ylabel){
...
weights_J400 <- df_data %>% select(date, Tickers, J400) %>%
...
# J200 weights
# create a xts of weights for each equity over time
weights_J200 <- df_data %>% select(date, Tickers, J200) %>%
        arrange(date) %>% group_by(Tickers) %>%
...
df_q3.1_data <- left_join(returns_J400 %>% mutate(cumulative_returns_J400 = cumprod(1+port_returns_J400)),
                        returns_J200 %>% mutate(cumulative_returns_J200 = cumprod(1+port_returns_J200)),
                        by = "date")

df_q3.1_data_tidy <- df_q3.1_data %>%
...
graph <-  ggplot(df_q3.1_data_tidy) +
