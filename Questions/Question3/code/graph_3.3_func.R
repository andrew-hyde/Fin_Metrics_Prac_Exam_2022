...
graph_q3.3_func <- function(df_data, title, subtitle, caption, xlabel, ylabel){
...
df_q3.3_data <- left_join(returns_J400 %>% mutate(cumulative_returns_J400 = cumprod(1+port_returns_J400)),
                        returns_J200 %>% mutate(cumulative_returns_J200 = cumprod(1+port_returns_J200)),
                        by = "date")

df_q3.3_data_tidy <- df_q3.3_data %>%
...
graph <-  ggplot(df_q3.3_data_tidy) +
