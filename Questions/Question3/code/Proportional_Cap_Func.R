...
Proportional_Cap_Func <- function(df_Cons, W_Cap){
    ...
    if(length(Breachers) > 0) {
        ...
        if( sum(df_Cons$weight, na.rm=T) > 1.0000001 | sum(df_Cons$weight, na.rm=T) < 0.9999999 | max(df_Cons$weight, na.rm = T) > W_Cap) {
